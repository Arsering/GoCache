...
#ifndef GRAPHSCOPE_UTILS_mmap_array_test_H_
#define GRAPHSCOPE_UTILS_mmap_array_test_H_
...
class mmap_array_test {
 public:
  ...
  mmap_array_test(const mmap_array_test& other) = delete;
  mmap_array_test& operator=(const mmap_array_test&) = delete;
#endif
  mmap_array_test(mmap_array_test&& rhs) : mmap_array_test() { swap(rhs); }
#if OV
  ~mmap_array_test() {
    if (fd_ != -1) {
...
#else
  // mmap_array_test(const mmap_array_test&) = delete;             // 阻止拷贝
  // mmap_array_test& operator=(const mmap_array_test&) = delete;  // 阻止赋值
  ~mmap_array_test() { close(); }
  ...
  void resize(size_t size) {
    ...
    if (read_only_) {
      if (size < size_) {
        ...
        LOG(FATAL) << "cannot resize read-only mmap_array_test to larger size "
                      "than file";
      }
    } else {
      if (data_ != NULL) {
  ...
  void resize(size_t size) {
    ...
    if (read_only_) {
      if (size < size_) {
        ...
        std::cerr << "cannot resize read-only mmap_array_test to larger size "
                     "than file";
      }
    } else {
...
#if OV
  void swap(mmap_array_test<T>& rhs) {
...
template <>
class mmap_array_test<std::string_view> {
 public:
  mmap_array_test() {}
  mmap_array_test(mmap_array_test&& rhs) : mmap_array_test() { swap(rhs); }
  ~mmap_array_test() {}
  ...
  void swap(mmap_array_test& rhs) {
    items_.swap(rhs.items_);
    data_.swap(rhs.data_);
  }

 private:
  mmap_array_test<string_item> items_;
  mmap_array_test<char> data_;
};

}  // namespace test

#endif  // GRAPHSCOPE_UTILS_mmap_array_test_H_
