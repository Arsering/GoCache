...
class FixedLengthColumnFamily {
 private:
  mmap_array<char> property_buffer_;
  size_t row_num_;
 ...
 public:
  FixedLengthColumnFamily() { gbp::GBPLOG << "cp"; }
  ~FixedLengthColumnFamily() { gbp::GBPLOG << "cp"; }
  ...
  void init(const std::vector<size_t>& ColumnLengths,
    ...
    offsets_.push_back(offset);
    property_buffer_.open(filename, false);
  }
...
#endif
    // 更新value
    property_buffer_.set(rowId * offsets_.back() + offsets_[columnId],
  ...
  gbp::BufferBlock getColumn(size_t rowId, size_t columnId) const {
    ...
    assert(columnId < offsets_.size());
#endif
    return property_buffer_.get(rowId * offsets_.back() + offsets_[columnId],
  ...
  // size的单位为byte
  void resize(size_t size) { property_buffer_.resize(size * offsets_.back()); }
};
