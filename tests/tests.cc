...
namespace test
{
  ...
  void read_bufferpool(size_t start_offset, size_t file_size_inByte, size_t io_size,
    size_t thread_id)
  {
    assert(io_size % sizeof(size_t) == 0);
    size_t io_num = file_size_inByte / io_size - 1;
    ...
    size_t st, page_id, item_num_firstpage, data, io_id;
    ...
    while (true)
    {
      ...
      // std::lock_guard lock(gbp::debug::get_file_lock());
      item_num_firstpage = (4096 - (curr_io_fileoffset % 4096)) / sizeof(size_t);
      page_id = curr_io_fileoffset / 4096;
      for (int i = 0;i < io_size / 4096; i++) {
        data = gbp::BufferObject::Ref<size_t>(ret, item_num_firstpage + (i - 1) * 512);
        if (data != page_id + i) {
          std::cout << data << " | " << page_id + i << std::endl;
        }
        assert(data == page_id + i);
      }
  ...
  void write_bufferpool(size_t start_offset, size_t file_size_inByte, size_t io_size,
    size_t thread_id)
  {
    ...
    for (io_id = 0; io_id < io_num; io_id++)
    {
      ...
      size_t buf_offset = 4096 - (curr_io_fileoffset % 4096 == 0 ? 4096 : curr_io_fileoffset % 4096);
      while (buf_offset < io_size) {
        page_id = (curr_io_fileoffset + buf_offset) / 4096;
        // memcpy(out_buf + buf_offset, &page_id, sizeof(size_t));
        gbp::BufferObject::UpdateContent<size_t>([&](size_t& content) {content = page_id;}, ret_obj, buf_offset / sizeof(size_t));

        // ::snprintf(out_buf + buf_offset, io_size - buf_offset, "%lu", page_id);
        buf_offset += 4096;
      }
      // auto ret = bpm.SetObject(out_buf, curr_io_fileoffset, io_size, 0, false);

      // if (*reinterpret_cast<size_t*>(ret.Data()) != io_id)
      // std::cout << *reinterpret_cast<size_t*>(out_buf) << " | " << io_id << std::endl;
      Client_Write_throughput().fetch_add(io_size);
    ...
    size_t thread_id)
  {
    ...
    size_t curr_io_fileoffset, ret, io_id, page_id;
    volatile size_t sum = 0;
    size_t st;
    for (io_id = 0; io_id < io_num; io_id++) {
      curr_io_fileoffset = io_id * io_size;
      ...
      {
        std::cout << *reinterpret_cast<size_t*>(in_buf) << " " << page_id << std::endl;
      }
  ...
  void write_pwrite(gbp::IOBackend* io_backend, size_t file_size_inByte, size_t io_size,
    size_t thread_id)
  {
    size_t io_num = gbp::ceil(file_size_inByte, io_size) - 1;
  ...
  void logging()
  {
    ...
    printf("%-25s%-25s%-25s%-25s%-25s%-25s%-25s\n", "Client_Read_Throughput", "Client_Write_Throughput", "SSD_Read_Throughput", "SSD_write_Throughput",
    ...
    while (true)
    {
      ...
      printf("%-25lf%-25lf%-25lf%-25lf%-25lu%-25lf\n",
        (cur_Client_Read_throughput - last_Client_Read_throughput) / (double)B2GB,
        (cur_Client_Write_throughput - last_Client_Write_throughput) / (double)B2GB,
  ...
  int test_concurrency(int argc, char** argv)
  {
    ...
    size_t pool_size_page = pool_size_MB * 1024LU * 1024LU / gbp::PAGE_SIZE_MEMORY / pool_num + 1;
