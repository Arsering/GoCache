...
namespace test
{
  ...
  void fiber_pread_0(gbp::DiskManager* disk_manager, size_t file_size_inByte, size_t io_size, size_t thread_id)
  {
    ...
    for (int idx = 0; idx < gbp::FIBER_BATCH_SIZE; idx++)
    {
      ...
      async_request_fiber_type* req = new async_request_fiber_type(in_buf, io_size, (gbp::fpage_id_type)fpage_id, 1, 0,
        context);
            ...
            while (!async_requests.empty()) {
              ...
              for (auto& req : async_requests) {
                ...
                if (process_func(req.value())) {
                  ...
                  context_type context = context_type::GetRawObject(&io_backend);
                  async_request_fiber_type* req_new = new async_request_fiber_type((char*)req.value()->io_vec[0].iov_base, io_size, (gbp::fpage_id_type)fpage_id, 1, 0,
                    context);
  ...
  void fiber_pread_1_1(gbp::DiskManager* disk_manager, size_t file_size_inByte, size_t io_size, size_t thread_id)
  {
    ...
    boost::lockfree::queue<async_request_fiber_type*, boost::lockfree::capacity<gbp::FIBER_CHANNEL_DEPTH>> async_channel;
  ...
  void fiber_pread_1_2(gbp::DiskManager* disk_manager, size_t file_size_inByte, size_t io_size, size_t thread_id)
  {
    ...
    for (int idx = 0; idx < gbp::FIBER_CHANNEL_DEPTH; idx++)
    {
      ...
      gbp::async_request_fiber_type* req = new gbp::async_request_fiber_type(in_buf, io_size, (gbp::fpage_id_type)fpage_id, 1, 0,
        context);
