...
//                 //   std::cout <<
//                 //
//                 // *reinterpret_cast<gbp::fpage_id_type*>(
...
//   gbp::fpage_id_type fpage_id;
//   bool* async_requests = (bool*) malloc(sizeof(bool) * FIBER_CHANNEL_DEPTH);
...
//     fpage_id = rnd(gen);
//     char* in_buf = (char*) aligned_alloc(gbp::PAGE_SIZE_FILE, io_size);
//     bufs.push_back(in_buf);

//     // ::memset(in_buf, 1, io_size);
//     as_atomic(async_requests[idx]) = false;
//     assert(server.SendRequest(0, fpage_id * gbp::PAGE_SIZE_FILE,
//                               gbp::PAGE_SIZE_FILE, bufs[idx],
//                               &(as_atomic(async_requests[idx])), true,
//                               true));
//   }

//   while (true) {
//     for (size_t idx = 0; idx < FIBER_CHANNEL_DEPTH; idx++) {
//       if (as_atomic(async_requests[idx])) {
//         // assert(*reinterpret_cast<gbp::fpage_id_type*>(
//         //            req.value()->io_vec[0].iov_base) ==
...
//         fpage_id = rnd(gen);
//         as_atomic(async_requests[idx]) = false;
//         assert(server.SendRequest(
//             0, fpage_id * gbp::PAGE_SIZE_FILE, gbp::PAGE_SIZE_FILE,
//             bufs[idx],
//             &(as_atomic(async_requests[idx])), true, true));
//       }
//     }
//   }
// }

// // void fiber_pread_2(gbp::DiskManager* disk_manager, size_t
// file_size_inByte,
// //                    size_t io_size, size_t thread_id) {
// //   std::random_device rd;
// //   std::mt19937 gen(rd());
...
//   gbp::fpage_id_type fpage_id;
//   for (int idx = 0; idx < FIBER_CHANNEL_DEPTH; idx++) {
//     fpage_id = rnd(gen);
//     char* in_buf = (char*) aligned_alloc(gbp::PAGE_SIZE_FILE, io_size);
//     assert(in_buf != nullptr);
...
//     // ::memset(in_buf, 1, io_size);
//     as_atomic(async_requests[idx]) = false;
//     io_server->SendRequest(0, fpage_id * gbp::PAGE_SIZE_FILE,
//                            gbp::PAGE_SIZE_FILE, bufs[idx],
//                            &(as_atomic(async_requests[idx])), true, true);
//   }
