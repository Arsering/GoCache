...
namespace gbp {
  ...
  bool BufferPool::FlushPage(fpage_id_type fpage_id, GBPfile_handle_type fd) {
    ...
    if (tar->dirty) {
      assert(ReadWrite(fpage_id * PAGE_SIZE_FILE, PAGE_SIZE_FILE, (char*)buffer_pool_->FromPageId(page_table_->ToPageId(tar)), PAGE_SIZE_MEMORY, tar->GetFileHandler(), false));
      tar->dirty = false;
    }
  ...
  bool BufferPool::ReadWrite(size_t offset, size_t file_size, char* buf, size_t buf_size,
    ...
    if constexpr (USING_FIBER_ASYNC_RESPONSE) {
      ...
      async_request_fiber_type* req = new async_request_fiber_type(
        buf, buf_size, offset, file_size, fd,
        context, is_read);
   ...
   */
  std::tuple<PTE*, char*> BufferPool::FetchPage(fpage_id_type fpage_id,
    GBPfile_handle_type fd) {
    assert(fpage_id <=
      ceil(disk_manager_->GetFileSizeShort(fd), PAGE_SIZE_MEMORY));
#ifdef DEBUG
    if (debug::get_log_marker() == 1)
  ...
  // }
    ...
    while (true) {
      ...
      case context_type::Phase::Evicting: {  // 2
        if (tar->dirty) {
          assert(ReadWrite(partitioner_->GetFPageIdGlobal(pool_ID_, tar->fpage_id) * PAGE_SIZE_FILE, PAGE_SIZE_FILE, mpage_data, PAGE_SIZE_MEMORY,
            tar->fd, false));
        }
      ...
      case context_type::Phase::Loading: {  // 4
        if constexpr (USING_FIBER_ASYNC_RESPONSE) {
          context_type context = context_type::GetRawObject();
          async_request_fiber_type* req = new async_request_fiber_type(
            mpage_data, PAGE_SIZE_MEMORY, (gbp::fpage_id_type)fpage_id * PAGE_SIZE_FILE, PAGE_SIZE_MEMORY, fd,
            context);
          assert(io_server_->SendRequest(req));
          ...
          io_server_->io_backend_->Read(fpage_id * PAGE_SIZE_FILE, mpage_data, PAGE_SIZE_FILE, fd);
        }
        ...
        assert(page_table_->CreateMapping(fd, fpage_id_inpool, page_table_->ToPageId(tar)));
  ...
  std::tuple<PTE*, char*> BufferPool::Pin(fpage_id_type fpage_id_inpool,
    GBPfile_handle_type fd) {
    // 1.1
    ...
    // if (gbp::debug::get_thread_id() == 1) {
    //   std::cout << "cpf" << std::endl;
    //   if (mpage_id == PageMapping::Mapping::BUSY_VALUE)
    //     std::cout << "a" << mpage_id << std::endl;
    //   else if (mpage_id == PageMapping::Mapping::EMPTY_VALUE)
    //     std::cout << "b" << mpage_id << std::endl;
    // }
    // if (gbp::debug::get_thread_id() == 0) {
    //   std::cout << "apf" << std::endl;
    //   if (mpage_id == PageMapping::Mapping::BUSY_VALUE)
    //     std::cout << "a" << mpage_id << std::endl;
    //   else if (mpage_id == PageMapping::Mapping::EMPTY_VALUE)
    //     std::cout << "b" << mpage_id << std::endl;
...
#endif
      object_size_t =
        PageTableInner::SetObject(buf, mpage, page_offset, object_size);
