...
namespace gbp
{
  ...
  void BufferPool::ReadWriteAsync(size_t offset, size_t file_size,
                                  ...
                                  AsyncMesg *finish)
  {
    if constexpr (IO_BACKEND_TYPE == 2)
    {
      assert(io_server_->SendRequest(fd, offset, file_size, buf, finish,
                                     is_read));
    }
    else
    {
                                                    ...
                                                    GBPfile_handle_type fd)
  {
    ...
    while (true)
    {
      ...
      case BP_async_request_type::Phase::Initing:
      { // 1.2
        mpage_id_type mpage_id;
      ...
      case BP_async_request_type::Phase::Evicting:
      { // 2
#ifdef DEBUG_BITMAP
      ...
      case BP_async_request_type::Phase::Loading:
      { // 4
  ...
  bool BufferPool::FetchPageSync1(BP_sync_request_type &req)
  {
    ...
    while (true)
    {
      ...
      case BP_sync_request_type::Phase::Initing:
      { // 1.2
        mpage_id_type mpage_id;
        if (free_list_->Poll(mpage_id))
        {
      ...
      case BP_sync_request_type::Phase::Evicting:
      { // 2
      ...
      case BP_sync_request_type::Phase::Loading:
      { // 4
        thread_local static PTE tmp;
        tmp.Clean();
