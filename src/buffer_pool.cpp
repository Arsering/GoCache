  ...
  while (true) {
    ...
    case context_type::Phase::Initing: {  // 1.2
      ...
      if (free_list_->Poll(mpage_id)) {
        ...
        // std::cout << "a" << page_table_->ToPageId(tar) << " | " << mpage_id
        // << std::endl; assert(ret); tar = page_table_->FromPageId(mpage_id);
        // if (!eviction_marker_)
        //   if (!eviction_server_->SendRequest(replacer_, free_list_, 10,
        //   &eviction_marker_))
        //     assert(false);
    ...
    case context_type::Phase::Loading: {  // 4
      if constexpr (USING_FIBER_ASYNC_RESPONSE) {
        gbp::context_type context = gbp::context_type::GetRawObject();
        gbp::async_request_fiber_type* req = new gbp::async_request_fiber_type(
            fpage_data, PAGE_SIZE_MEMORY, (gbp::fpage_id_type) fpage_id, 1, fd,
            context);
        assert(io_server_->SendRequest(req));

        // auto [success, req] = io_server_->SendRequest(fd, fpage_id, 1,
        // fpage_data);
        size_t loops = 0;
        while (!req->success) {
          // hybrid_spin(loops);
      ...
      if (!page_table_->CreateMapping(fd, fpage_id_inpool,
