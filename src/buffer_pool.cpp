...
pair_min<PTE*, char*> BufferPool::FetchPageSync(fpage_id_type fpage_id,
  ...
  while (true) {
    ...
    case BP_async_request_type::Phase::Evicting: {  // 2
      ...
      // gbp::get_thread_logfile() << key << std::endl;
      // CacheInfo::GetCacheInfo().Insert(ret.first->fd_cur,
      //                                  ret.first->fpage_id_cur);
    ...
    case BP_async_request_type::Phase::Loading: {  // 4
      ...
      *reinterpret_cast<fpage_id_type*>(ret.second) = fpage_id;
      ...
      // CacheInfo::GetCacheInfo().CheckExists(fd, fpage_id);
      // gbp::get_thread_logfile() << "miss" << std::endl;
...
pair_min<PTE*, char*> BufferPool::LockPage(fpage_id_type fpage_id,
  ...
  // GBPLOG << ret.first->ref_count << " " << ret.first->fpage_id_cur;
