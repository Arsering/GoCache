 ...
 */
void BufferPoolManager::init(uint16_t pool_num, size_t pool_size_page,
                             uint16_t io_server_num,
                             const std::string& file_path) {
  pool_num_ = pool_num;
  get_pool_num().store(pool_num);
  pool_size_page_ = pool_size_page;
  ...
  for (int idx = 0; idx < pool_num; idx++) {
    ...
    pools_[idx]->init(idx, pool_size_page, io_servers_[idx % io_server_num],
