  ...
  struct alignas(sizeof(uint64_t)) PTE {
    ...
    std::tuple<bool, uint16_t> IncRefCount(fpage_id_type fpage_id,
      ...
      do {
        ...
        auto query_id = debug::get_query_id().load();
        // if (new_unpacked.fd == 22 && new_unpacked.fpage_id == 1)
        //   LOG(INFO) << "cp" << query_id;
    ...
    std::tuple<bool, uint16_t> DecRefCount(bool is_write = false) {
      ...
      do {
        ...
        // if (new_header.ref_count <= 0)
        //   LOG(INFO) << "aa: " << new_header.fd << " " << new_header.fpage_id;
        // if (new_header.fd == 22 && new_header.fpage_id == 1)
        //   LOG(INFO) << "cp";
...
class PageTable {
 public:
  ...
  FORCE_INLINE std::tuple<bool, mpage_id_type> FindMapping(
    ...
    assert(fd < mappings_.size());
    if (fpage_id >= mappings_[fd]->Size()) {
      LOG(FATAL) << fd << " " << mappings_[fd]->Size() << " " << fpage_id;
    }
  ...
  FORCE_INLINE std::tuple<bool, mpage_id_type> LockMapping(
      GBPfile_handle_type fd, fpage_id_type fpage_id, bool for_create = true) {
    assert(fd < mappings_.size());
