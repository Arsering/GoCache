...
namespace gbp {
  ...
  public:
    ...
    struct alignas(sizeof(uint64_t)) PTE {
      ...
      // 无需获得文件页的相关信息，因为该内存页的 ref_count >0 时不可能被用于存储其他文件页
      std::tuple<bool, uint16_t> DecRefCount(bool is_write = false, bool write_to_ssd = false) {
        std::atomic<uint64_t>& atomic_packed = as_atomic(AsPacked());
        uint64_t old_packed = atomic_packed.load(std::memory_order_relaxed),
          new_packed;
        uint16_t old_ref_count;
  ...
  public:
    ...
    /**
     * @brief 本函数只会在两种情况下被调用：1. 文件页加载进/驱逐出内存时 2. 文件页被flush出内存
     *
     * @param fd
     * @param fpage_id
    ...
    FORCE_INLINE std::tuple<bool, mpage_id_type> LockMapping(
      ...
      if (mpage_id == PageMapping::Mapping::EMPTY_VALUE) return { true, mpage_id };

      auto* tar = FromPageId(mpage_id);
      // 在mapping被锁住之前，有其他正常的访问到达了pte，导致pte的锁住失败
      if (!tar->Lock()) {
        assert(mappings_[fd]->CreateMapping(fpage_id, mpage_id));// 一旦锁pte失败，则必须释放
        return  { false, 0 };
      }
      // 一旦mapping被锁住，那说明tar->fpage_id != fpage_id && tar->fd == fd是一定会成立的，所以无需测试
    ...
    FORCE_INLINE bool UnLockMapping(
      GBPfile_handle_type fd, fpage_id_type fpage_id, mpage_id_type mpage_id) {
      ...
      assert(fd < mappings_.size());
      if (!mappings_[fd]->CreateMapping(fpage_id, mpage_id)) return false;
