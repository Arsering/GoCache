...
namespace gbp
{
  ...
  {
  public:
    struct SSD_IO_request_type
    {
      SSD_IO_request_type() = default;
      SSD_IO_request_type(char *buf, size_t buf_size, size_t _offset, size_t _file_size,
                          GBPfile_handle_type _fd, AsyncMesg *_finish, bool _read = true) : io_vec(1), file_offset(_offset), file_size(_file_size), fd(_fd), finish(_finish), read(_read)
      {
    ...
    virtual bool Read(size_t offset, char *data, size_t size,
                      GBPfile_handle_type fd, AsyncMesg *finish = nullptr) = 0;
    virtual bool Read(size_t offset, const ::iovec *io_info, size_t count,
                      GBPfile_handle_type fd, AsyncMesg *finish = nullptr) = 0;
    virtual void BatchReadWrite(const std::vector<SSD_IO_request_type> &requests) = 0;
  ...
  public:
    ...
    bool Read(size_t offset, const ::iovec *io_info, size_t count,
              GBPfile_handle_type fd, AsyncMesg *finish = nullptr) override
    {
#if ASSERT_ENABLE
      assert(fd < disk_manager_->fd_oss_.size() &&
    ...
    void BatchReadWrite(const std::vector<SSD_IO_request_type> &requests) override
    {
      for (auto req_id = 0; req_id < requests.size(); req_id++)
      {
        while (!Read(requests[req_id].file_offset, requests[req_id].io_vec.data(), requests[req_id].io_vec.size(), requests[req_id].fd, requests[req_id].finish))
          ;
        ...
        // io_uring_prep_readv(
        //     sqe,                                               // 用这个 SQE 准备一个待提交的 read 操作
        //     disk_manager_->fd_oss_[requests[req_id].fd].first, // 从 fd 打开的文件中读取数据
        //     requests[req_id].io_vec.data(),                    // iovec 地址，读到的数据写入 iovec 缓冲区
        //     requests[req_id].io_vec.size(),                    // iovec 数量
        //     requests[req_id].file_offset);                     // 读取操作的起始地址偏移量
        // io_uring_sqe_set_data(sqe, requests[req_id].finish);
        // num_preparing_++;
      }
      Progress();
    }
  ...
  public:
    ...
    bool Read(size_t offset, const ::iovec *io_info, size_t io_count,
              GBPfile_handle_type fd, AsyncMesg *finish = nullptr) override
    {
#if ASSERT_ENABLE
      assert(fd < disk_manager_->fd_oss_.size() &&
    ...
    void BatchReadWrite(const std::vector<SSD_IO_request_type> &requests) {}
