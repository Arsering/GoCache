...
namespace gbp
{
  ...
  public:
    ...
    FORCE_INLINE GBPfile_handle_type OpenFile(const std::string& file_path,
      int o_flag = O_RDWR | O_CREAT | O_DIRECT)
    {
  ...
  public:
    ...
    virtual bool Write(size_t offset, std::string_view data, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;
    virtual bool Write(size_t offset, char* data, size_t size, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;
    virtual bool Write(fpage_id_type fpage_id, ::iovec* io_info, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;

    virtual bool Read(fpage_id_type fpage_id, void* data, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;
    virtual bool Read(size_t offset, std::string_view data, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;
    virtual bool Read(size_t offset, char* data, size_t size, GBPfile_handle_type fd,
      bool* finish = nullptr) = 0;
  ...
  public:
      ...
      bool* finish = nullptr) override
    {
      ...
      ::fdatasync(disk_manager_->fd_oss_[fd].first); // needs to flush to keep disk file in sync
      ...
      bool* finish = nullptr) override
    {
      ...
      auto ret = ::pwrite(disk_manager_->fd_oss_[fd].first, data.data(), data.size(),
        offset);
      ...
      bool* finish = nullptr) override
    {
      ...
      auto ret = ::pwrite(disk_manager_->fd_oss_[fd].first, data, size,
        offset);
      ...
      bool* finish = nullptr) override
    {
      ...
      size_t offset = (size_t)fpage_id * PAGE_SIZE_FILE;
      auto ret = ::pwrite(disk_manager_->fd_oss_[fd].first, io_info[0].iov_base, PAGE_SIZE_FILE,
        offset);
      ...
      bool* finish = nullptr) override
    {
      ...
      auto ret =
        ::pread(disk_manager_->fd_oss_[fd].first, (void*)data.data(), data.size(), offset);
      ...
      bool* finish = nullptr) override
    {
      ...
      auto ret =
        ::pread(disk_manager_->fd_oss_[fd].first, data, size, offset);
      // if (ret != size)
      // {
    ...
    bool
      Read(fpage_id_type fpage_id, ::iovec* io_info, GBPfile_handle_type fd,
        bool* finish = nullptr) override
    {
      assert(fd < disk_manager_->fd_oss_.size() &&
        disk_manager_->fd_oss_[fd].second);
