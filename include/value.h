...
class Value {
  ...
  void Malloc(size_t s) {
    if (s <= STACK_SIZE) {
      ...
      if (data_)
        std::cerr << "Allocation failed" << s << std::endl;
      need_delete_ = true;
      size_ = s;
    }
  }
...
#pragma GCC diagnostic pop
  ...
  T_Obj& Obj() {
    if (size_ != sizeof(T_Obj)) {
      std::cerr << "size_!=sizeof(T_Obj)" << std::endl;
      exit(-1);
    }
    return *reinterpret_cast<T_Obj*>(data_);
  }
   ...
   * \return  A Value.
   */
  static Value ConstRef(const std::string& s) {
   ...
   * \return  A Value.
   */
  static Value ConstRef(const char* const& s) {
    return Value(s, std::strlen(s));
  }
...
#define ENABLE_RVALUE(RT)   \
  ...
  typename std::enable_if<  \
      std::is_same<typename std::decay<VALUE>::type, Value>::value, RT>::type
}  // namespace gbp
