...
class LruReplacer : public Replacer<TKey>{
  /**
   * The LRU list node.
   *
   ...
   * 这里的TValue是用户自己想要的数据，剩下的部分负责维护HashTable和ListNode之间的一致性
   * 这个东西的问题在于HashTable我们可能也会选择修改
   * 这里有没有可能构造一个HashMap的抽象接口
   */
  struct HashMapValue {
    HashMapValue() : m_listNode(nullptr) {}

    HashMapValue(ListNode* node)
        : m_listNode(node) {}
  ...
  typedef const TKey SnapshotValue;

 public:
  /**
   * Create a container with a given maximum size
   */
  explicit LruReplacer(size_t maxSize);
  ...
  void Insert(const TKey &value);
  bool Victim(TKey &value);
  bool Erase(const TKey &value);
  size_t Size();
   ...
   * returned.
   */
  bool insert(const TKey& key);//Insert
 ...
 private:
   ...
   * this is called.
   */
  void delink(ListNode* node);//erase
...
template <class TKey>
typename LruReplacer<TKey>::ListNode* const
    LruReplacer<TKey>::OutOfListMarker = (ListNode*)-1;

//初始化函数
template <class TKey>
LruReplacer<TKey>::LruReplacer(size_t maxSize)
    : m_maxSize(maxSize),
      m_size(0),
...
template <class TKey>
bool LruReplacer<TKey>::find(const TKey& key) {
...
template <class TKey>
bool LruReplacer<TKey>::insert(const TKey& key) {
  ...
  HashMapValuePair hashMapValue(key, HashMapValue(node));
...
template <class TKey>
void LruReplacer<TKey>::snapshotKeys(
...
template <class TKey>
inline void LruReplacer<TKey>::delink(ListNode* node) {
...
template <class TKey>
inline void LruReplacer<TKey>::pushFront(ListNode* node) {
...
template <class TKey>
void LruReplacer<TKey>::delete_key(const TKey& key) {
