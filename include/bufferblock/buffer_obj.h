...
class BufferBlockIter {
 public:
  ...
  BufferBlockIter(const BufferBlock& buffer_obj) {
    ...
    cur_page_num_rest_ =
        (PAGE_SIZE_MEMORY - ((uintptr_t) cur_ptr_ % PAGE_SIZE_MEMORY)) /
        sizeof(OBJ_Type);
    cur_page_num_rest_--;
  }
  ~BufferBlockIter() = default;
  ...
  FORCE_INLINE OBJ_Type* next() {
    if (GS_likely(cur_page_num_rest_ > 0)) {
      cur_page_num_rest_--;
      cur_ptr_++;
    } else {
      cur_page_id_++;
      if (cur_page_id_ >= buffer_obj_->page_num_) {
        ...
        cur_ptr_ = reinterpret_cast<OBJ_Type*>(
            buffer_obj_->datas_.datas[cur_page_id_]);
