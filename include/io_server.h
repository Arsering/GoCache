...
struct context_type {
  ...
  FORCE_INLINE static context_type GetRawObject() {
    return {Type::Pin, Phase::Begin, State::Commit, false};
  }
};
...
struct async_request_fiber_type {
  ...
  async_request_fiber_type(std::vector<::iovec>& _io_vec, size_t _offset,
                           ...
                           context_type& _async_context, bool _read = true)
      : file_offset(_offset),
...
class IOServer_old {
 public:
  IOServer_old(DiskManager* disk_manager)
  ...
  // std::tuple<bool, PointerWrapper<async_request_fiber_type>>
  // SendRequest(GBPfile_handle_type fd, fpage_id_type fpage_id_start,
  // fpage_id_type page_num, char* buf, bool blocked = true)
  // {
  //   assert(buf != nullptr);
  //   context_type context = context_type::GetRawObject();
  //   auto* req = new async_request_fiber_type(buf, PAGE_SIZE_FILE,
  //   fpage_id_start, 1, fd, context); SendRequest(req, blocked); return {
  //   SendRequest(req, blocked), req };
  // }

  std::tuple<bool, std::shared_ptr<async_request_fiber_type>> SendRequest(
      GBPfile_handle_type fd, size_t offset, size_t size, char* buf,
      bool blocked = true) {
    assert(buf != nullptr);
    context_type context = context_type::GetRawObject();
    std::shared_ptr<async_request_fiber_type> req(new async_request_fiber_type(
        buf, PAGE_SIZE_FILE, offset, size, fd, context));
...
class IOServer {
 public:
  IOServer(gbp::DiskManager* disk_manager) : stop_(false) {
    ...
    else if constexpr (gbp::IO_BACKEND_TYPE == 2) {
 ...
 private:
  bool process_func(async_request_fiber_type* req) {
    switch (req->async_context.state) {
    case context_type::State::Commit: {  // 将read request提交至io_uring
      auto ret = io_backend_->Read(req->file_offset, req->io_vec.data(),
                                   req->fd, &req->async_context.finish);
      while (!ret) {
        ret = io_backend_->Read(
            req->file_offset, req->io_vec.data(), req->fd,
